...
namespace Macro.ImageViewer.Utilities.Print.Dicom.Preview.View.Winforms
{
    ...
    /// </summary>
    public partial class PrintImageBoxControl : UserControl
    {
        ...
        internal PrintViewImageBox ImageBox
        {
        ...
        internal Rectangle ParentRectangle
        {
            ...
            set
            {
                ...
                float scale =
                    ScaleToFit(new Size((int)(_imageBox.NormalizedRectangle.Width * 10), (int)(_imageBox.NormalizedRectangle.Height * 10)), _parentRectangle.Size);
                width = (int)(_imageBox.NormalizedRectangle.Width * 10 * scale);
                height = (int)(_imageBox.NormalizedRectangle.Height * 10 * scale);

                x = (_parentRectangle.Width - width) / 2;
                y = (_parentRectangle.Height - height) / 2;
        ...
        private IEnumerable<PrintTileControl> TileControls
        {
            get
            {
                foreach (Control control in this.Controls)
                {
                    if (control is PrintTileControl)
                        yield return (PrintTileControl)control;
                }
            }
        }

        private PrintTileControl GetTileControl(ITile tile)
        {
            foreach (PrintTileControl tileControl in TileControls)
            {
        ...
        private void DoDraw()
        {
            foreach (PrintTileControl control in this.TileControls)
                control.Draw();
            Invalidate();
        }
        ...
        private void DisposeControls(IEnumerable<PrintTileControl> controls)
        {
            foreach (PrintTileControl control in controls)
            {
        ...
        {
            List<PrintTileControl> oldControls = new List<PrintTileControl>(this.TileControls);
        ...
        private void DrawImageBoxBorder(PaintEventArgs e)
        {
            ...
            DrawBorder(
                ...
                PrintViewImageBox.BorderWidth,
                PrintViewImageBox.InsetWidth);
        }
            ...
            if (this.Controls.Count > 1)
            {
                foreach (PrintTileControl control in this.TileControls)
                {
                    ...
                    DrawBorder(
                        ...
                        Tile.BorderWidth,
                        Tile.InsetWidth);// control.Tile.isSubTile ? 1 : Tile.InsetWidth
                }
            }
        }

        private Rectangle GetTileRectangle(PrintTileControl control)
        {
        ...
        {
            PrintTileControl tileControl = sender as PrintTileControl;
        ...
        {
            foreach (PrintTileControl control in this.TileControls)
                control.SetParentImageBoxRectangle(this.AvailableClientRectangle, PrintViewImageBox.InsetWidth, suppressDraw);
        }
        ...
        public Rectangle AvailableClientRectangle
        {
            get
            {
                ...
                if (ImageScrollerVisible)
                    clientRectangle.Width -= _imageScroller.Width - PrintViewImageBox.InsetWidth;
        ...
        private void ImageScrollerMouseDown(object sender, EventArgs e)
        {
            if (_imageBox != null)
            {
                if (_imageBox.Tiles.Count > 0)
                {
                    ...
                    PrintTileControl tileControl = GetTileControl(_imageBox.SelectedTile);
                    if (tileControl != null)
                        tileControl.Focus();
        ...
        private void ImageScrollerValueChanged(object sender, PrintTrackSlider.ValueChangedEventArgs e)
        {
            if (e.UserAction == PrintTrackSlider.UserAction.None)
            {
        ...
        private void UpdateImageScroller()
        {
            ...
            if (displaySet != null)
            {
                ...
                int maximum = Math.Max(0, _imageBox.TotleImageCount - tileCount);
                if (maximum > 0)
                {
                    visibleNow = true;
